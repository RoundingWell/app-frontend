...
function syncIframeCoverage() {
  cy
    ...
    .then(iframeCoverage => {
      cy
        .task('combineCoverage', JSON.stringify(iframeCoverage), { log: false });
    });
}
...
context('Formservice', function() {
  ...
  specify('action formservice fetches responses by action and by tag', function() {
    cy.then(() => {
      cy
  ...
  specify('formio loads action and standalone form definitions', function() {
    cy.then(() => {
      cy
        .intercept('GET', '/api/actions/1/form', {
          statusCode: 200,
