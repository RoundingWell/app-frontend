...
context('worklist page', function() {
  specify('toggle filters sidebar', function() {
    ...
    cy
      .get('.patient-list-page__selection [role="checkbox"]')
      .then($selectAll => {
        cy
          .get('.worklist-list__check [role="checkbox"]')
          .first()
          .should($checkbox => {
            expect($checkbox[0].getBoundingClientRect().left)
              .to.equal($selectAll[0].getBoundingClientRect().left);
          });
      });
  });
