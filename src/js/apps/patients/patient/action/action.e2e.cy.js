...
context('patient action page', { scrollBehavior: 'center' }, function() {
  specify('expands a form after canceling the first action load', function() {
    ...
    cy.get('.patient-action-loading__skeleton').should('not.exist');
    cy.get('.patient-action').should('not.exist');

    // Leaving after the action renders must also cancel its unfinished activity owner.
    let releaseActivity;
    cy.intercept({ method: 'GET', url: '/api/actions/**/activity*', times: 1 }, req => {
      return new Cypress.Promise(resolve => {
        releaseActivity = () => {
          req.reply({ body: { data: [] } });
          resolve();
    ...
    cy.get('.patient-action__activity-loading').should('exist');
    cy.wrap(null).should(() => expect(releaseActivity).to.be.a('function'));
    cy.navigate('/patient/1/workflow');
    cy.get('.patient-action').should('not.exist').then(() => releaseActivity());
    cy.wait('@canceledActivity');
    cy.waitForAppRequests();
    cy.get('.patient-action').should('not.exist');
    cy.navigate(`/patient/1/action/${ action.id }`);
  ...
  specify('action phone dialer', function() {
    ...
    cy.intercept({ method: 'GET', url: '/api/patients/**/fields/phones', times: 1 }, {
      statusCode: 422,
      body: { errors: [{ status: '422', title: 'Unable to load phone numbers' }] },
    }).as('failedPhones');

    cy.get('.patient-action [data-dialer-region] button').as('actionDialerButton').click();
    cy.wait('@failedPhones');
    cy.get('.picklist').should('not.exist');
    cy.get('@actionDialerButton').should('be.enabled');
    ...
    cy.intercept({ method: 'GET', url: '/api/patients/**/fields/phones', times: 1 }, req => {
      markPhonesRequested();
      req.on('response', () => phonesResponse);
    }).as('delayedPhones');

    cy.get('@actionDialerButton').click();
    cy.then(() => phonesRequested);
    cy.get('.picklist__message-loading').should('be.visible');
    cy.get('body').type('{esc}');
    cy.get('.picklist').should('not.exist');
    cy.then(() => releasePhones());
    cy.wait('@delayedPhones');
    cy.waitForAppRequests();
    cy.get('.picklist').should('not.exist');
