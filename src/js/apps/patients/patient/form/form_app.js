...
export default App.extend({
  ...
  async prepareStart(options, { signal }) {
    ...
    if (!actionId) {
      return Promise.all([
        tagRequestFailure('form', Radio.request('entities', 'fetch:forms:model', formId, { signal })),
        null,
        tagRequestFailure('responses', Radio.request('entities', 'fetch:formResponses:byMe', { patientId: patient.id, formId }, { signal })),
      ]);
    }

    return Promise.all([
      tagRequestFailure('form', Radio.request('entities', 'fetch:forms:byAction', actionId, { signal })),
      tagRequestFailure('action', Radio.request('entities', 'fetch:actions:withResponses', actionId, { signal })),
      tagRequestFailure('responses', Radio.request('entities', 'fetch:formResponses:byMe', { actionId }, { signal })),
    ]);
  },
